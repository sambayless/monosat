...
void TreeReachDetector::addLit(int from, int to, Var reach_var){
	g.invalidate();
	antig.invalidate();
...
void TreeReachDetector::preprocess(){
	//vec<bool> pure;
...
void TreeReachDetector::buildReachReason(int node,vec<Lit> & conflict){
		...
		void TreeReachDetector::buildNonReachReason(int node,vec<Lit> & conflict){
			static int it = 0;
			++it;
			int u = node;
		 ...
		 */
		void TreeReachDetector::buildForcedEdgeReason(int reach_node, int forced_edge_id,vec<Lit> & conflict){
					static int it = 0;
					++it;
		...
		void TreeReachDetector::buildReason(Lit p, vec<Lit> & reason, CRef marker){
		...
		bool TreeReachDetector::propagate(vec<Assignment> & trail,vec<Lit> & conflict){
...
bool TreeReachDetector::checkSatisfied(){
...
void TreeReachDetector::dbg_sync_reachability(){
		for(int j = 0;j< reach_lits.size();j++){
						Lit l =reach_lits[j];
						if(l!=lit_Undef){
							int node = getNode(var(l));
...
int TreeReachDetector::OptimalWeightEdgeStatus::operator [] (int edge) const {
...
int TreeReachDetector::OptimalWeightEdgeStatus::size()const{
	return detector.outer->edge_list.size();
}


Lit TreeReachDetector::decide(){
