...
template<typename PT, typename D>
void quickhull3D<PT, D>::findMinMax(uint & q0, uint & q1, uint & q2) const {
	q0 = 1;
	q1 = 1;
	q2 = 1;
	uintc imax = pts.size();
...
void quickhull3D<PT, D>::reset() {
	...
	HSC * h1 = new HSC(HS(&pts[0], x0, x1, x2), pts);
	//new HSC( HS(pts[x0],pts[x1],pts[x2]) ,pts );
	h1->isInsideOrOnBoundary(index);
	cs.push_back(h1);
	
	HSC * h2 = new HSC(HS(pts[x2], pts[x1], pts[x0]), pts);
...
template<typename PT, typename D>
void quickhull3D<PT, D>::partition(HSC & h) {
//cout << "partition" << endl;
...
//cout << SHOW(print(target)) << endl;
	...
	HSC* h1 = new HSC(HS(pts[k], h.halfspace.p1, h.halfspace.p2), pts);
	//h1->isInsideOrOnBoundary(target);
	h1->subtractfrom(target);
	HSC* h2 = new HSC(HS(pts[k], h.halfspace.p0, h.halfspace.p1), pts);
	//h2->isInsideOrOnBoundary(target);
	h2->subtractfrom(target);
	HSC* h3 = new HSC(HS(pts[k], h.halfspace.p2, h.halfspace.p0), pts);
