...
public:
	...
	//{ point2<T> z(*this); z-=p; return z; } seg faulted?
			{
		return point2<T>(this->x - p.x, this->y - p.y);
	} // seg faulted too.
...
public:
	...
	static void readin(vector<point3<T> >& v, T const * abeg, T const * aend);
	...
	void crossproduct(point3<T> const & u, point3<T> const & v) {
		x = u.y * v.z - v.y * u.z;
		y = u.z * v.x - u.x * v.z;
		z = u.x * v.y - u.y * v.x;
	}
...
template<class T>
void point3<T>::readin(vector<point3<T> >& v, T const * abeg, T const * aend) {
