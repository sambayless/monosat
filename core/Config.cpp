...
BoolOption Minisat::opt_partial(_cat,"partial","Attempt to find a partial satisfying solution",true);

BoolOption Minisat::opt_fast_partial(_cat,"fast-partial","Find cheaper (but worse) partial solutions",true);
 ...
 BoolOption Minisat::opt_allsat(_cat_allsat,"allsat","Perform allsat",true);
 IntOption Minisat::opt_allsat_from(_cat_allsat,"from", "First variable to include in the allsat set (note: variables are numbered starting at 1)", 1, IntRange(1,INT32_MAX));
 IntOption Minisat::opt_allsat_to(_cat_allsat,"to", "Last variable (inclusive) to include in the allsat set", INT32_MAX, IntRange(1,INT32_MAX));
 ...
 BoolOption Minisat::opt_allsat_modsat(_cat_allsat,"allsat-modsat","Use modular sat solver for allsat",false);
 BoolOption Minisat::opt_decide_allsat_first(_cat_allsat,"decide-allsat-first","When not using the modular SAT solver for allsat, force the SAT solver to decide the allsat variables first",false);
